...
<body onload="window.print()"> <div class="no-print" style="position: fixed; top: 0; right: 0; padding: 10px; background: white; border: 1px solid #ccc; z-index: 1000;">
    ...
    <div class="header text-center">
        @if(isset($settings['company_logo']) && file_exists(public_path('storage/'.$settings['company_logo'])))
    ...
    @php $subtotal = $order->details->sum(function($d){ return $d->quantity * $d->price; }); @endphp
    <div class="totals">
        <div class="row">
            <span>Subtotal:</span>
            <span>{{ $settings['currency_symbol'] ?? 'S/' }} {{ number_format($subtotal, 2) }}</span>
        </div>
