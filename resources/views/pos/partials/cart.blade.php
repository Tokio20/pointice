<div class="flex-grow-1 overflow-auto p-2">
    @php $cartItemCount = $order ? $order->details->sum('quantity') : 0; @endphp
...
<div class="bg-white border-top p-3 flex-shrink-0">
    @if($order)
        @php $subtotal = $order->details->sum(function($d){ return $d->price * $d->quantity; }); @endphp
        <input type="hidden" id="cartTotalValue" value="{{ number_format($order->total, 2, '.', '') }}">
        <input type="hidden" id="cartItemCount" value="{{ $cartItemCount }}">
        ...
        <div class="row g-1 mb-2">
            ...
            <div class="col-12 col-sm-4 mb-2 mb-sm-0">
                <a href="{{ route('pos.split.content', $order->id) }}" class="btn btn-light w-100 border text-muted btn-sm btn-lg fw-bold py-2" title="Dividir">
                    <i class="bi bi-scissors"></i> <span style="font-size: 0.85rem;">Div.</span>
                </a>
            </div>
            <div class="col-12 col-sm-4">
                <a href="{{ route('pos.precheck', $order->id) }}" target="_blank" class="btn btn-light w-100 border text-muted btn-sm btn-lg fw-bold py-2" title="Pre-cuenta">
