            ...
            <div class="d-md-none mt-2 px-2 w-100">
                <div class="d-flex overflow-auto gap-2">
                    ...
                    @foreach($categories as $category)
                        <button class="btn btn-sm btn-outline-secondary" onclick="filterProducts('cat-{{ $category->id }}')">{{ $category->name }}</button>
                    @endforeach
                </div>
            </div>
            ...
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3 pb-5">
                @foreach($categories as $category)
                    @foreach($category->products as $product)
                        <div class="col product-item cat-{{ $category->id }}" data-name="{{ strtolower($product->name) }}">
                            <div class="card h-100 border-0 shadow-sm product-card" onclick="addToOrder({{ $product->id }})" style="cursor: pointer; transition: transform 0.1s; touch-action: manipulation;">
                                <div class="position-relative">
                                        @if($product->image)
                                        <img src="{{ asset('storage/'.$product->image) }}" class="card-img-top" style="height: 140px; object-fit: cover;">
                                    @else
                                ...
                                <div class="card-body p-3 text-center">
                                    <h6 class="card-title fs-5 mb-0 text-truncate">{{ $product->name }}</h6>
                                </div>
                            </div>
                        </div>
                    @endforeach
...
<div class="d-md-none fixed-bottom bg-white border-top p-2">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            ...
            <div class="fw-bold">{{ $currency ?? 'S/' }}<span id="bottomTotal">{{ number_format($order->total ?? 0, 2) }}</span></div>
        </div>
        <div>
            <button class="btn btn-primary btn-lg" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasCart" aria-controls="offcanvasCart">
                Ver Cuenta (<span id="bottomCount">{{ $order ? $order->details->sum('quantity') : 0 }}</span>)
            </button>
        </div>
    </div>
</div>
